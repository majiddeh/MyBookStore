...
public class JalaliCalendar extends Calendar {
    ...
    public JalaliCalendar(TimeZone zone, Locale aLocale) {
        ...
        gDate gDate = new gDate(calendar.get(YEAR), calendar.get(MONTH), calendar.get(DATE));
        gDate = MiladiToJalali(gDate);
        set(gDate.getYear(), gDate.getMonth(), gDate.getDay());
        complete();
    ...
    JalaliCalendar(int year, int month, int dayOfMonth,
        ...
        gDate gDate = jalaliToMiladi(new gDate(fields[1], fields[2], fields[5]));
        cal = new GregorianCalendar(gDate.getYear(), gDate.getMonth(), gDate.getDay(), hourOfDay,
                minute, second);
    ...
    public static gDate MiladiToJalali(gDate gregorian) {
        ...
        return new gDate(jalaliYear, jalaliMonth, jalaliDay);
    }
    ...
    public static gDate jalaliToMiladi(gDate jalali) {
        ...
        return new gDate(gregorianYear, gregorianMonth, gregorianDay);
    ...
    protected void computeTime() {
        ...
        if (!isTimeSet && !isTimeSeted) {
            ...
            gDate gDate = jalaliToMiladi(new gDate(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH)));
            cal.set(gDate.getYear(), gDate.getMonth(), gDate.getDay()
            ...
            gDate gDate = jalaliToMiladi(new gDate(internalGet(YEAR), internalGet(MONTH), internalGet(DAY_OF_MONTH)));
            cal.set(gDate.getYear(), gDate.getMonth(), gDate.getDay(), internalGet(HOUR_OF_DAY),
            ...
            case DST_OFFSET: {
                if (isSet(YEAR) && isSet(MONTH) && isSet(DATE) && isSet(HOUR) && isSet(HOUR_OF_DAY) &&
                    ...
                    cal.set(gDate.getYear(), gDate.getMonth(), gDate.getDay(), internalGet(HOUR_OF_DAY), internalGet(MINUTE),
                            internalGet(SECOND));
                    cal.set(field, value);
                    gDate = MiladiToJalali(new gDate(cal.get(YEAR), cal.get(MONTH), cal.get(DATE)));
                    super.set(YEAR, gDate.getYear());
                    super.set(MONTH, gDate.getMonth());
                    super.set(DATE, gDate.getDay());
                    super.set(HOUR_OF_DAY, cal.get(HOUR_OF_DAY));
                    super.set(MINUTE, cal.get(MINUTE));
                    super.set(SECOND, cal.get(SECOND));
    ...
    protected void computeFields() {
        ...
        if (!areFieldsSet) {
            ...
            //Day_of_Week
            super.set(DAY_OF_WEEK, dayOfWeek(jalaliToMiladi(new gDate(fields[1], fields[2], fields[5]))));
            //***
    ...
    public void add(int field, int amount) {
        ...
        if (field == MONTH) {
            ...
            gc.add(field, amount);
            gDate = MiladiToJalali(new gDate(gc.get(YEAR), gc.get(MONTH), gc.get(DATE)));
            super.set(YEAR, gDate.getYear());
            super.set(MONTH, gDate.getMonth());
            super.set(DATE, gDate.getDay());
